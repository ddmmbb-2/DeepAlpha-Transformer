...
parser.add_argument('--max_stocks', type=int, default=500, help='必須與特徵工程中的股票數一致')
parser.add_argument('--seq_len', type=int, default=60)
parser.add_argument('--batch_size', type=int, default=4)
parser.add_argument('--gru_hidden', type=int, default=192)
parser.add_argument('--embed_dim', type=int, default=32, help='股票實體嵌入維度')
...
# 滾動驗證參數
parser.add_argument('--train_window', type=int, default=1000, help='訓練窗口大小(交易日)')
parser.add_argument('--step_size', type=int, default=250, help='滾動向前推進步長(交易日)')
...
# ================== 1. 升級版 Dataset (支援解包海外特徵) ==================
class StockDataset(Dataset):
    def __init__(self, features, mask, labels, macro_features, start_idx, end_idx, seq_len):
        self.features = features
        self.mask = mask
        self.labels = labels
        self.macro_features = macro_features  # 全局總經特徵 (T, G)
    ...
    def __getitem__(self, idx):
        ...
        x = self.features[t_start:t_end]   # (seq_len, N, F)
        m = self.mask[t_start:t_end]       # (seq_len, N)
        y = self.labels[t_end - 1]          # (N,)
        
        # 提取預測當天（即 60 天時間序列的最後一天）的海外宏觀快照
        macro_x = self.macro_features[t_end - 1]  # (G,)
        
        return torch.FloatTensor(x), torch.FloatTensor(m), torch.LongTensor(y), torch.FloatTensor(macro_x)
...
class ScaledGRUTransformer(nn.Module):
    def __init__(self, num_stocks, feat_dim, macro_dim, gru_hidden, embed_dim, nhead, num_layers, dropout):
        super().__init__()
        ...
        # 🛠️ 方法 B 核心：將 (時序特徵 + 個股身分 + 全局宏觀特徵) 投影壓回 gru_hidden
        self.project = nn.Linear(gru_hidden + embed_dim + macro_dim, gru_hidden)
    ...
    def forward(self, x, mask, macro_x):
        B, S, N, F = x.shape
        ...
        last_feat = gru_out[:, :, -1, :]  # (B, N, gru_hidden)
        
        # 2. 獲取個股 Embedding 身份身份
        stock_ids = torch.arange(N, device=x.device).unsqueeze(0).expand(B, N)
        embed_feat = self.stock_embed(stock_ids)  # (B, N, embed_dim)
        
        # 3. 🛠️ 方法 B：橫向擴展全局宏觀特徵 (B, G) -> (B, N, G)
        G = macro_x.shape[-1]
        macro_feat = macro_x.unsqueeze(1).expand(B, N, G)
        
        # 4. 三合一拼接並低負擔投影
        combined = torch.cat([last_feat, embed_feat, macro_feat], dim=-1) # (B, N, gru_hidden + embed_dim + G)
        transformer_input = self.project(combined)                       # (B, N, gru_hidden)
        
        # 5. 送入截面 Transformer 計算市場連動
...
def evaluate(model, dataloader, top_k, device):
    ...
    total_pos = 0
    with torch.no_grad():
        for x, m, y, mx in dataloader:
            x, m, y, mx = x.to(device), m.to(device), y.to(device), mx.to(device)
            logits = model(x, m, mx)
...
def main():
    ...
    macro_features = data_dict['macro_features']  # 載入新海外特徵 (T, G)

    T, N_full, F = features.shape
    G = macro_features.shape[1]
    ...
    print(f"✅ 資料完整性檢查通過。原始天數 T={T}, 股票數 N={N}, 個股特徵 F={F}, 宏觀特徵 G={G}")

    if args.use_focal:
        print("採用損失函數: Focal Loss (gamma=2.0)")
        loss_fn = FocalLoss(gamma=2.0)
    else:
        print(f"採用損失函數: BCEWithLogitsLoss (正樣本權重={args.pos_weight})")
        # 修正：改名為 base_loss_fn，隔絕命名空間衝突
    ...
    print(f"\n=======================================================")
    print(f"啟動海外宏觀天眼選股模型 | 訓練窗口: {train_window} 天 | 步長: {step_size} 天")
    print(f"=======================================================")
    ...
    for start_t in range(0, T - train_window, step_size):
        ...
        print(f"\n========== 🔮 Fold {fold} ==========")
        print(f"訓練區間: [{start_t}, {train_end-1}] | 驗證區間: [{train_end}, {val_end-1}] | 測試區間: [{val_end}, {test_end-1}]")

        # 建立具備宏觀特徵的 Dataloader
        train_set = StockDataset(features, mask, labels, macro_features, start_t, train_end, args.seq_len)
        val_set   = StockDataset(features, mask, labels, macro_features, train_end, val_end, args.seq_len)
        test_set  = StockDataset(features, mask, labels, macro_features, val_end, test_end, args.seq_len)

        train_loader = DataLoader(train_set, batch_size=args.batch_size, shuffle=True)
        val_loader   = DataLoader(val_set, batch_size=args.batch_size, shuffle=False)
        test_loader  = DataLoader(test_set, batch_size=args.batch_size, shuffle=False)

        # 重置模型權重，防止 Look-ahead
        model = ScaledGRUTransformer(
            num_stocks=N, feat_dim=F, macro_dim=G,
        ...
        best_val_hit = 0.0
        fold_ckpt_dir = os.path.join(args.checkpoint_dir, f'fold_{fold}')
        os.makedirs(fold_ckpt_dir, exist_ok=True)
        best_model_path = os.path.join(fold_ckpt_dir, 'best_model.pth')

        # 訓練迴圈 (為方便你隨時看進度，每一輪皆列印資訊)
        for epoch in range(args.epochs):
            model.train()
            train_loss = 0.0
            nan_batches = 0
            for x, m, y, mx in train_loader:
                x, m, y, mx = x.to(device), m.to(device), y.to(device), mx.to(device)
                optimizer.zero_grad()
    ...
    print(f"\n=======================================================")
    print(f"🏅 全新升級版 DeepAlpha-Transformer 綜合盲測報告")
