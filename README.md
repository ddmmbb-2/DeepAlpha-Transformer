...
本系統的核心創舉在於融合了 **GRU 的時間序列特徵提取能力** 與 **Transformer 的全市場跨截面（Cross-Sectional）多頭注意力機制**，並開創性地引入**可訓練實體嵌入（Entity Embeddings）**，賦予模型對個股屬性的長效記憶，最終透過極其嚴謹的 **5-Fold 滾動式驗證（Walk-Forward Validation）** 進行盲測評估。
...
### 3. 不平衡樣本優化 (Focal Loss & Positive Weighting)
系統將預測目標定義為「未來 20 個交易日超額累積報酬率位居全市場前 20% 的強勢股」。面對 1:4 的天生類別不平衡，系統深度整合了 **Focal Loss** 與正樣本加權機制，公式如下：

$$FL(p_t) = -\alpha_t (1 - p_t)^\gamma \log(p_t)$$

透過動態調降易分類負樣本的權重，強迫模型將注意力聚焦在真正具有 Alpha 爆發潛力的非線性波動標的。

### 4. 嚴謹的滾動式驗證 (5-Fold Walk-Forward Validation)
拒絕傳統機器學習在金融時序數據上不切實際的靜態切分（Static Split）。本專案完全實作了滾動時間窗口（Sliding Window）機制。每個 Fold 的模型皆會**徹底重置權重**，並將前 1000 天作為訓練、緊接其後的 125 天作為驗證、最後 125 天作為嚴格的 Out-of-Sample (OOS) 盲測測試。透過多輪跨越不同年份的牛熊市考驗，徹底杜絕潛在的看前預知偏差（Look-ahead Bias）。
...
├── add.py                # 特徵模組：流動性篩選與Expanding因子工程
├── v1.py                 # 訓練模組：滾動式驗證與模型核心架構
├── infer.py              # 推論模組：最新 Top-K 強勢股推薦生成
│
...
在 `add.py` 中，系統會自發性篩選全市場平均成交量最大的 **500 檔高流動性標的**，並建構 12 項核心量價衍生特徵：
...
執行數據下載模組，系統會自動切分 Batch 並啟動隨機延遲（Anti-429 速率限制）：
...
融合批次 CSV 檔案，進行流動性前 500 檔篩選、擴展標準化與未來 20 日 Alpha 標籤計算：
...
### Step 4. 啟動 5-Fold 滾動式訓練 (Windows 環境命令)

使用以下指令啟動具備 Entity Embeddings 與 Focal Loss 的滾動驗證流程：
...
載入最新一期（當前天數前 60 天）的特徵快照，配合受訓完成的模型進行截面勝率預測，輸出實戰名單：

```bash
python infer.py --data processed_data/features.npz --model checkpoints/fold_5/best_model.pth --top_k 20

```

預測完成後，系統會自動在根目錄生成 `recommendations.csv`，依據超額報酬機率由大到小排序，做為每日實盤選股的重要參考依據。
