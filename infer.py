...
# 💡 預設改為指向 V5 的權重檔（可手動動態指定如 checkpointsV2/fold_1_best.pth）
parser.add_argument('--model', type=str, default='checkpoints/fold_1_best.pth', help='模型權重路徑')
parser.add_argument('--seq_len', type=int, default=60)
parser.add_argument('--gru_hidden', type=int, default=384, help='V5 大腦：隱含層 384')
...
if not os.path.exists(args.model):
    raise FileNotFoundError(f"❌ 找不到模型權重：'{args.model}'，請檢查檔名或使用 --model 指定正確路徑")
...
with torch.no_grad():
    ...
    # 💡 關鍵優化：推論端同樣開啟 autocast，確保數值空間與 V5 訓練時高度咬合
